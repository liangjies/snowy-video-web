	...
	<view>
				...
				<view :style="'width: '+ Width +'px; display: flex; flex-direction: row;'">
					...
					<view :style="'width: '+ (Width*0.8) +'px; display: flex; flex-direction: column; margin-top: 20px; margin-left: 10px;'">
						...
						<view :style="'width: '+ (Width*0.8) +'px; height: 20px; display: flex; flex-direction: row; margin-top: 12.5px;'">
							...
							<image v-if="!list.mylike" @click="tolike(index)" src="@/static/douyin/dianzan-7.png" style="width: 17.5px; height: 17.5px; position: absolute; right: 22.5px; margin-left: 2.5px;"></image>
							<image v-if="list.mylike" @click="tolike(index)" src="@/static/douyin/dianzan-6.png" style="width: 17.5px; height: 17.5px; position: absolute; right: 22.5px; margin-left: 2.5px;"></image>
							...
							<view :style="'width: '+ (Width*0.8) +'px; display: flex; flex-direction: row;'">
								...
								<view :style="'width: '+ (Width*0.8*0.85) +'px; display: flex;  flex-direction: column; margin-top: 20px; margin-left: 10px;'">
									<view style="display: flex; flex-direction: row;">
										...
										<!-- 13.图片是一个向右的小图标 -->
										<image src="@/static/douyin/you-3.png" style="width: 10px; height: 10px; margin-top: 2.5px; margin-left: 5px;"></image>
										<!-- 14.被回复用户的名称 会显示在这里 -->
									...
									<view :style="'width: '+ (Width*0.8*0.85) +'px; height: 20px; display: flex; flex-direction: row; margin-top: 12.5px;'">
										...
										<image v-if="!pinlun_list[index].sonPinlun[inde].mylike" @click="tosonlike(index,inde)" src="@/static/douyin/dianzan-7.png" style="width: 17.5px; height: 17.5px; position: absolute; right: 10px; margin-left: 2.5px;"></image>
										<image v-if="pinlun_list[index].sonPinlun[inde].mylike" @click="tosonlike(index,inde)" src="@/static/douyin/dianzan-6.png" style="width: 17.5px; height: 17.5px; position: absolute; right: 10px; margin-left: 2.5px;"></image>
		...
		<view v-if="show && platform=='ios'" @touchstart="openPinglun" :style="'width: '+ Width +'px; height: '+ plHeight +'px; background-color: #000000; border-top: 1px solid #4a494b;'">
			<view :style="'width: '+ (Width-30) +'px; height: 42px; margin-left: 15px; margin-top: 5px; background-color: #181818; border-radius: 50px; display: flex; flex-direction: row;'">
				...
				<image src="@/static/douyin/biaoqing.png" style="width: 25px; margin-top: 7.5px; height: 25px; position: absolute; right: 12.5px;"></image>
			</view>
		</view>
			...
			<view style="display: flex; flex-direction: column;">
				<view @click="openPinglun" :style="'width: '+ Width +'px; background-color: #FFFFFF; display: flex; flex-direction: row;'">
					<view :style="'width: '+ (Width-30)*percent +'px; margin-left: 15px; margin-top: 10px; background-color: #f2f0f4; border-radius: '+ borderRadius +'px; margin-bottom: 10px; display: flex; flex-direction: row;'">
						...
						<!-- 10.点击表情展开评论框 -->
						<image v-if="!isopen" @click="toemoji" src="@/static/douyin/biaoqing-2.png" style="width: 25px; margin-top: 7.5px; height: 25px; position: absolute; right: 12.5px;"></image>
						<image v-if="isopen" @click="toemoji" src="@/static/douyin/jianpan.png" style="width: 25px; margin-top: 7.5px; height: 25px; position: absolute; right: 12.5px;"></image>
					</view>
					<!-- 11.发送按钮
					...
					<view v-if="(value!=='' || imageURL !== '') && isSend" @click="sendSMS" style="width: 30px; height: 30px; border-radius: 40px; background-color: #ff1a63; margin-top: 15px; margin-left: 10px;">
						<image src="@/static/douyin/shangyi.png" style="width: 15px; height: 15px; margin-top: 8px; margin-left: 8px;"></image>
					</view>
				</view>
			</view>
			<!-- 12.用于显示用户选择的 GIF 图片 -->
			...
			*/ 
						...
						for(let i=0;i<pin.length;i++){
							if(pin[i].sonPinlun.length !== 0){
								...
								for(let j=0;j<List.length;j++){
									let likeinf = List[j].like.indexOf(uni.getStorageSync("user")._id);
									if(likeinf !== -1){
							...
							success: (resa) => {
								...
								for(let i=0;i<pins.length;i++){
									if(pins[i].sonPinlun.length !== 0){
										...
										let likeinfo = pins[i].like.indexOf(uni.getStorageSync("user")._id);
										if(likeinfo !== -1){
											pins[i].mylike = true;
										} else {
											pins[i].mylike = false;
										...
										pins[i].sonpinlunText = "展开" + pins[i].sonPinlun.length + "条回复";
			...
			tolike(index){
				...
				if(this.pinlunList[index].mylike == false){
					this.pinlunListX[(this.pinlunListX.length-1)-index].like.push(uni.getStorageSync("user")._id);
				} else {
					var sms = []
					for(let i=0;i<this.pinlunListX[(this.pinlunListX.length-1)-index].like.length;i++){
						if(this.pinlunListX[(this.pinlunListX.length-1)-index].like[i] !== uni.getStorageSync("user")._id){
			...
			tosonlike(index,inde){
				...
				if(this.pinlun_list[index].sonPinlun[inde].mylike == false){
					this.pinlunListX[(this.pinlunListX.length-1)-index].sonPinlun[(this.pinlunListX[(this.pinlunListX.length-1)-index].sonPinlun.length-1)-inde].like.push(uni.getStorageSync("user")._id);
				} else {
					var sms = []
					for(let i=0;i<this.pinlunListX[(this.pinlunListX.length-1)-index].sonPinlun[(this.pinlunListX[(this.pinlunListX.length-1)-index].sonPinlun.length-1)-inde].like.length;i++){
						if(this.pinlunListX[(this.pinlunListX.length-1)-index].sonPinlun[(this.pinlunListX[(this.pinlunListX.length-1)-index].sonPinlun.length-1)-inde].like[i] !== uni.getStorageSync("user")._id){
