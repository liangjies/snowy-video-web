	...
	export default {
		...
		methods: {
			...
			getUserMsg() {
				...
				if (user.backgroundImage != null && user.backgroundImage != '' && user
			...
			changeAvatar() {
				this.uploadImage('头像上传中...', '/user/uploadAvatar?userId=', 'avatar', '头像修改成功!',
					'头像修改失败')
			},
			changeBackgroundImage() {
				this.uploadImage('背景上传中...', '/user/uploadBackgroundImage?userId=',
					'backgroundImage', '背景修改成功!', '背景修改失败')
			},
			// 修改用户信息
			...
			changeNickName() {
				...
				this.changeUserInfo(this.tempNickName, '/user/updateNickName?userId=',
					'昵称修改成功!', '昵称修改失败', () => {
			...
			changeSignature() {
				...
				this.changeUserInfo(this.tempSignature, '/user/updateSignature?userId=',
					'签名修改成功!', '签名修改失败', () => {
			...
			changeGender() {
				...
				this.changeUserInfo(tempGender, '/user/updateGender?userId=', '性别修改成功!',
					'性别修改失败', () => {
						if (tempGender === 1) {
							this.gender = {
								type: '男',
